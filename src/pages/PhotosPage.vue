...
<template >
  <Card class="container mx-auto gap-3">
    <template #content>
      ...
      <form @submit="searchImages" class="gap-2 mx-2">
        ...
        placeholder="Select a race" class="w-full" /> -->
          <Select v-model="raceId" :options="races" optionLabel="Name" optionValue="id" @change="onChangeEvent"
            placeholder="Select a race" class="md:w-14rem w-full" />
        </div>

        <div class="card flex justify-content-center w-full gap-2 py-1"
          v-if="!races.find(x => x.id == raceId)?.photoStatus.includes('faceonly')">
          ...
          <AutoComplete id="searchBib" v-model="bibSelection" showClear :suggestions="items" @complete="searchBib"
            placeholder="Enter your BIB number" :dropdown-click="searchBib" class="w-full" inputClass="px-2" />
      ...
      <div v-if="!bibSelection && raceId && races.find(x => x.id == raceId)?.photoStatus.includes('face')">
        <hr />
        <FileUpload mode="basic" name="demo[]" accept="image/*" customUpload @uploader="faceUploader" class="w-full"
          chooseLabel="Upload cropped face image 👤" :maxFileSize="10000000" :auto="true" />
      </div>
    ...
    <template #content>
      <div class="flex flex-wrap bg-gray-200 w-full justify-evenly">
        ...
        <div v-for="(img, i) in images">
          <Image v-tooltip="'Click to see high resolution image'" class="p-image thumb flex-auto m-2 "
            v-bind:aria-label="images[i].imagePath" v-bind:src="getPublicUrl('thumbs', raceId, images[i].imagePath)"
      ...
      <div class="container py-10 px-10 mx-0 min-w-full flex flex-col items-center" v-if="raceId">
        ...
        <div class="w-full text-center justify-center flex-col" v-if="images.length && (bibNo || uploadedImage)">
  ...
  <Dialog v-model:visible="visible" maximizable modal v-bind:header="dialogHeader">

    <Image v-tooltip="'Click to download'" :src="getPublicUrl('processed', raceId, images[entryToEdit].imagePath)"
      v-bind:aria-label="images[entryToEdit].imagePath" />

    <div style="font-size: 1rem">
      <ShareNetwork network="facebook" :url="shareableUrl" title="Celebrating fitness"
        ...
        quote="Never give up!" hashtags="#pcmcrunners">
        <Button><i class="pi pi-share-alt" /> &nbsp;<i class="pi pi-facebook"></i></Button>
      </ShareNetwork>

      <ShareNetwork network="WhatsApp" :url="shareableUrl" title="Celebrating fitness"
      ...
      <a :href="getPublicUrl('processed', raceId, images[entryToEdit].imagePath)"
        :download="`RUNPIX_{raceId}_{entryToEdit.vale}`">
